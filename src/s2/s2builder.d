...
private:
  ...
  void simplifyEdgeChains(
    ...
    foreach (ref edges; layer_edges) edges.length = 0;
    foreach (ref input_edge_ids; layer_input_edge_ids) input_edge_ids.length = 0;
...
private:
  ...
  bool isInterior(VertexId v) {
    ...
    for (auto i = 0; i < edges.length; ) {
      ...
      for (; i < edges.length && graphEdgeLayer(edges[i]) == layer; ++i) {
        ...
        if (edge[0] == v) matcher.tally(edge[1], true /*outgoing*/);
        if (edge[1] == v) matcher.tally(edge[0], false /*outgoing*/);
      }
      if (!matcher.matches()) {
        return false;
      }
  ...
  void assignDegenerateEdges(
    ...
    foreach (ref ids; merged_ids) sort(ids);
...
private bool isFullPolygonUnspecified(in Graph g, ref S2Error error) {
