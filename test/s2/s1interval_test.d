...
enum double PI = to!double(math.PI);
enum double PI_2 = to!double(math.PI_2);
...
const S1Interval pi2 = S1Interval(PI_2, PI_2);
const S1Interval pi = S1Interval(PI, PI);
const S1Interval mipi = S1Interval(-PI, -PI);  // Same as "pi" after normalization.
const S1Interval mipi2 = S1Interval(-PI_2, -PI_2);
// Single quadrants:
const S1Interval quad1 = S1Interval(0, PI_2);
const S1Interval quad2 = S1Interval(PI_2, -PI);
const S1Interval quad3 = S1Interval(PI, -PI_2);
const S1Interval quad4 = S1Interval(-PI_2, 0);
// Quadrant pairs:
const S1Interval quad12 = S1Interval(0, -PI);
const S1Interval quad23 = S1Interval(PI_2, -PI_2);
const S1Interval quad34 = S1Interval(-PI, 0);
const S1Interval quad41 = S1Interval(-PI_2, PI_2);
// Quadrant triples:
const S1Interval quad123 = S1Interval(0, -PI_2);
const S1Interval quad234 = S1Interval(PI_2, 0);
const S1Interval quad341 = S1Interval(PI, PI_2);
const S1Interval quad412 = S1Interval(-PI_2, -PI);
...
const S1Interval mid12 = S1Interval(PI_2 - 0.01, PI_2 + 0.02);
const S1Interval mid23 = S1Interval(PI - 0.01, -PI + 0.02);
const S1Interval mid34 = S1Interval(-PI_2 - 0.01, -PI_2 + 0.02);
...
unittest {
  ...
  Assert.equal(quad12.hi(), PI);
  Assert.equal(quad34[0], PI);
  Assert.equal(quad34[1], 0);
  Assert.equal(quad34.bounds(), Vector2_d(PI, 0));
  Assert.equal(pi.lo(), PI);
  Assert.equal(pi.hi(), PI);

  // Check that [-Pi, -Pi] is normalized to [Pi, Pi].
  Assert.equal(mipi.lo(), PI);
  Assert.equal(mipi.hi(), PI);
  Assert.equal(quad23.lo(), PI_2);
  Assert.equal(quad23.hi(), -PI_2);
...
unittest {
  ...
  const double kAlmostPi = PI - 2 * double.epsilon;
  Assert.equal(S1Interval(-kAlmostPi, PI).isFull(), false);
  Assert.equal(S1Interval(-PI, kAlmostPi).isFull(), false);
  Assert.equal(S1Interval(PI, -kAlmostPi).isEmpty(), false);
  Assert.equal(S1Interval(kAlmostPi, -PI).isEmpty(), false);
}

@("GetCenter")
unittest {
  Assert.equal(quad12.getCenter(), PI_2);
  Assert.approximately(S1Interval(3.1, 2.9).getCenter(), 3.0 - PI, DOUBLE_ERR);
  Assert.approximately(S1Interval(-2.9, -3.1).getCenter(), PI - 3.0, DOUBLE_ERR);
  Assert.approximately(S1Interval(2.1, -2.1).getCenter(), PI, DOUBLE_ERR);
  Assert.equal(pi.getCenter(), PI);
  Assert.equal(mipi.getCenter(), PI);
  Assert.equal(math.fabs(quad23.getCenter()), PI);
  Assert.approximately(quad123.getCenter(), 0.75 * PI, DOUBLE_ERR);
}

@("GetLength")
unittest {
  Assert.equal(quad12.getLength(), PI);
  Assert.equal(pi.getLength(), 0);
  Assert.equal(mipi.getLength(), 0);
  Assert.approximately(quad123.getLength(), 1.5 * PI, DOUBLE_ERR);
  Assert.equal(math.fabs(quad23.getLength()), PI);
  Assert.equal(full.getLength(), 2 * PI);
...
unittest {
  ...
  Assert.equal(
      !empty.contains(0) && !empty.contains(PI) && !empty.contains(-PI), true);
  Assert.equal(!empty.interiorContains(PI) && !empty.interiorContains(-PI), true);
  Assert.equal(full.contains(0) && full.contains(PI) && full.contains(-PI), true);
  Assert.equal(full.interiorContains(PI) && full.interiorContains(-PI), true);
  Assert.equal(
      quad12.contains(0) && quad12.contains(PI) && quad12.contains(-PI), true);
  Assert.equal(quad12.interiorContains(PI_2) && !quad12.interiorContains(0), true);
  Assert.equal(
      !quad12.interiorContains(PI) && !quad12.interiorContains(-PI), true);
  Assert.equal(quad23.contains(PI_2) && quad23.contains(-PI_2), true);
  Assert.equal(quad23.contains(PI) && quad23.contains(-PI), true);
  Assert.equal(!quad23.contains(0), true);
  Assert.equal(
      !quad23.interiorContains(PI_2) && !quad23.interiorContains(-PI_2), true);
  Assert.equal(quad23.interiorContains(PI) && quad23.interiorContains(-PI), true);
  Assert.equal(!quad23.interiorContains(0), true);
  Assert.equal(pi.contains(PI) && pi.contains(-PI) && !pi.contains(0), true);
  Assert.equal(!pi.interiorContains(PI) && !pi.interiorContains(-PI), true);
  Assert.equal(mipi.contains(PI) && mipi.contains(-PI) && !mipi.contains(0), true);
  Assert.equal(!mipi.interiorContains(PI) && !mipi.interiorContains(-PI), true);
...
unittest {
  ...
  testIntervalOps(zero, pi, "FFFF", S1Interval(0, PI), empty);
  ...
  testIntervalOps(pi2, pi, "FFFF", S1Interval(PI_2, PI), empty);
  ...
  testIntervalOps(pi, zero, "FFFF", S1Interval(PI, 0), empty);
  testIntervalOps(pi, pi, "TFTF", pi, pi);
  testIntervalOps(pi, pi2, "FFFF", S1Interval(PI_2, PI), empty);
  testIntervalOps(pi, mipi, "TFTF", pi, pi);
  testIntervalOps(pi, mipi2, "FFFF", quad3, empty);
  testIntervalOps(pi, quad12, "FFTF", S1Interval(0, PI), pi);
  ...
  testIntervalOps(mipi, mipi2, "FFFF", S1Interval(-PI, -PI_2), empty);
  ...
  testIntervalOps(quad23, quad34, "FFTT", quad234, S1Interval(-PI, -PI_2));

  testIntervalOps(quad1, quad23, "FFTF", quad123, S1Interval(PI_2, PI_2));
...
unittest {
  ...
  Assert.equal(r, zero);
  r = empty; r.addPoint(PI);
  Assert.equal(r, pi);
  r = empty; r.addPoint(-PI);
  Assert.equal(r, mipi);
  r = empty; r.addPoint(PI); r.addPoint(-PI);
  Assert.equal(r, pi);
  r = empty; r.addPoint(-PI); r.addPoint(PI);
  Assert.equal(r, mipi);
  ...
  r = quad1; r.addPoint(-0.9*PI); r.addPoint(-PI_2);
  Assert.equal(r, quad123);
  r = full; r.addPoint(0);
  Assert.equal(r.isFull(), true);
  r = full; r.addPoint(PI);
  Assert.equal(r.isFull(), true);
  r = full; r.addPoint(-PI);
  Assert.equal(r.isFull(), true);
}
...
@("Project")
unittest {
  S1Interval r = S1Interval(-PI, -PI);
  Assert.equal(PI, r.project(-PI));
  Assert.equal(PI, r.project(0));
  r = S1Interval(0, PI);
  Assert.equal(0.1, r.project(0.1));
  Assert.equal(0.0, r.project(-PI_2 + 1e-15));
  Assert.equal(PI, r.project(-PI_2 - 1e-15));
  r = S1Interval(PI - 0.1, -PI + 0.1);
  Assert.equal(PI, r.project(PI));
  Assert.equal(PI - 0.1, r.project(1e-15));
  Assert.equal(-PI + 0.1, r.project(-1e-15));
  Assert.equal(0.0, S1Interval.full().project(0));
  Assert.equal(PI, S1Interval.full().project(PI));
  Assert.equal(PI, S1Interval.full().project(-PI));
}

@("FromPointPair")
unittest {
  Assert.equal(S1Interval.fromPointPair(-PI, PI), pi);
  Assert.equal(S1Interval.fromPointPair(PI, -PI), pi);
...
unittest {
  ...
  Assert.equal(mipi.expanded(0.01), S1Interval(PI - 0.01, -PI + 0.01));
  Assert.equal(pi.expanded(27), full);
  Assert.equal(pi.expanded(PI_2).approxEquals(quad23), true);
  Assert.equal(pi2.expanded(PI_2), quad12);
  Assert.equal(mipi2.expanded(PI_2), quad34);

  Assert.equal(empty.expanded(-1), empty);
  Assert.equal(full.expanded(-1), full);
  Assert.equal(quad123.expanded(-27), empty);
  Assert.equal(quad234.expanded(-27), empty);
  Assert.equal(quad123.expanded(-PI_2), quad2);
  Assert.equal(quad341.expanded(-PI_2).approxEquals(quad4), true);
  Assert.equal(quad412.expanded(-PI_2), quad1);
}
...
unittest {
  ...
  Assert.equal(S1Interval(PI - kLo, -PI + kLo).approxEquals(empty), true);
  ...
  Assert.equal(S1Interval(-PI + kLo, PI - kLo).approxEquals(full), true);
  Assert.equal(S1Interval(-PI, PI - 2*kHi).approxEquals(full), false);

  // Singleton intervals.
  Assert.equal(pi.approxEquals(pi) && mipi.approxEquals(pi), true);
  Assert.equal(pi.approxEquals(S1Interval(PI - kLo, PI - kLo)), true);
  Assert.equal(pi.approxEquals(S1Interval(PI - kHi, PI - kHi)), false);
  Assert.equal(pi.approxEquals(S1Interval(PI - kLo, -PI + kLo)), true);
  Assert.equal(pi.approxEquals(S1Interval(PI - kHi, -PI)), false);
  ...
  Assert.equal(S1Interval(PI - 0.5 * kLo, -PI + 0.5 * kLo)
      .approxEquals(S1Interval(-PI + 0.5 * kLo, PI - 0.5 * kLo)), false);
...
unittest {
  ...
  Assert.approximately(PI, mid12.getDirectedHausdorffDistance(empty), FLOAT_ERR);
